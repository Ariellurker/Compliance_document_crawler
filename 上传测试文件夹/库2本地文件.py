...
def sync_wiki_to_local(tenant_access_token: str, local_path: str, space_id: str = "", parent_node_token:str = "") -> Exception:
    """递归同步知识库到本地
    ...
    Args:
        ...
        space_id: 知识空间节点
    """
    print(f"开始同步知识库: {space_id or '根目录'} -> {local_path}")
    ...
    for file_info in cloud_files:
        ...
        file_type = file_info.get("obj_type", "")
        file_token = file_info.get("obj_token", "")
        modified_time = file_info.get("obj_edit_time", "0")
        
        if not file_name or not file_token:
            continue
            
        if file_info["has_child"] == True:
            # 递归处理子文件夹
            local_item_path = os.path.join(local_path, file_name)
            sub_folder_token = file_info["node_token"]
            sync_wiki_to_local(tenant_access_token, local_item_path, space_id, sub_folder_token)
...
if __name__ == "__main__":
    ...
    try:
        # 开始同步
        sync_wiki_to_local(tenant_access_token, local_root_path, space_id or "")
        print("同步完成!")
    except Exception as e:
